...
public class SECourseManager {
	 ...
	 * @param detailId
	 * @param date
	 * @param recordJson json数组
	 * @param cb
	 */
	public void createPlanRecord(String userId, String planId, String detailId, String date, String recordJson, final Callback<CommentSucessResult> callback) {
		getCourseService().createPlanRecord(userId, planId, detailId, date, recordJson, new Callback<CommentSucessResult>() {
			@Override
			public void success(CommentSucessResult result, Response response) {
				if (callback != null) {
					callback.success(result, response);
